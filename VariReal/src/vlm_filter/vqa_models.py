...
class SBERTModel:
    def __init__(self, ckpt="sentence-transformers/all-MiniLM-L6-v2", cache_dir=None):
        self.tokenizer = AutoTokenizer.from_pretrained(ckpt, cache_dir=cache_dir)
        self.model = AutoModel.from_pretrained(ckpt, cache_dir=cache_dir)
        self.model.eval()
...
class VQAModelFiltering:
    def __init__(self, model_name_or_path: str, load_in_Nbit: Union[int, None] = None, logger=None, cache_dir=None):
        self.model = ModelFactory(model_name=model_name_or_path, load_in_Nbit=load_in_Nbit, cache_dir=cache_dir)
        self.sbert_model = SBERTModel(cache_dir=cache_dir)
        self.logger = logger
    ...
    def get_tifa_score(self, question_answer_pairs, image, enable_logging=False):
        ...
        if enable_logging:
            ...
            self.logger.log(f"Sampled vqa answers (out of {len(vqa_output_list)}) :\n{formatted_vqa_output_list}")
